...
class BanditLearn:
    ...
    def epsilonGreedyLearnMultipleRuns(self, numberOfRuns, numberOfPulls, isStationary=True, eps=0.5, alpha=-1):
        ...
        for run in range(numberOfRuns):
            ...
            self.reset()
            learnResults = self.epsilonGreedyLearn(numberOfPulls, isStationary, eps, alpha, -1 )
    ...
    def optimisticLearnMultipleRuns(self, numberOfRuns, numberOfPulls, initialEstimates, isStationary=True, eps=0.5,
                                    alpha=-1):
        avgRewardVector = np.array([0.0] * numberOfPulls)
        optimalActionVector = np.array([0.0] * numberOfPulls)
        for run in range(numberOfRuns):
